...
def _review_from_row(row: Any, run: AgentRunRecord) -> DevReviewRecord:
    ...
    return DevReviewRecord(
        ...
        main_work_threads=_json_loads(data["main_work_threads"], []),
        shipping_progress=_json_loads(data["shipping_progress"], []),
        collaboration_load=_json_loads(data["collaboration_load"], []),
        meeting_load=_json_loads(data["meeting_load"], []),
        rhythm_risks=_json_loads(data["rhythm_risks"], []),
        next_week_suggestion=data["next_week_suggestion"],
        source_coverage=_json_loads(data["source_coverage"], {}),
