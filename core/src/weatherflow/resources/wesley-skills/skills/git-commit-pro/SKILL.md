---
name: git-commit-pro
description: Write well-structured git commit messages following the Conventional
  Commits standard. Use when a user asks to commit changes, write a commit message,
  stage and commit, improve a commit message, or follow commit conventions. Analyzes
  diffs to produce accurate, meaningful commit messages.
license: Apache-2.0
metadata:
